...
def help(message):
    bot.send_message(
        ...
        "Нет нужной информации? Напишите нам: @Dreams_and_lights",parse_mode="HTML")
...
def start(message):
    bot.send_message(
        message.chat.id,
        "Не знаете как провести время с детьми? Мы поможем!\n\n"
        "<b>Мы - команда студентов из ВШЭ и сейчас тестируем нашу идею приложения"
        " для отцов, которым не всегда хватает времени и сил на времяпрепровождение со своими детьми.\n\n"
        "Суть идеи:\n Раз в несколько дней Вам будет присылаться задание, Вам необходимо будет выполнить"
        " это задание и отправить фотографию с прогулки. В конце тестирования идеи вы сможете получить"
        " фотоальбом из ваших фотографий.</b>"
        "\n\n<i>Информация по командам:</i>\n"
        "Получили задание? Чтобы сдать задание (отправить фотографии), напишите /pass\nПосле отправки фотографий, напишите /got\n"
        "Чтобы получить альбом, напишите /send_album\n"
...
def pass_task(message):
    bot.send_message(
        message.chat.id,
        "Отправьте, пожалуйста, фотографии выполнения задания, а также описание вашего проведенного вместе времени"
        " в одном сообщении. После сообщения с фотографиями пришлите /got"
    )
    pictures.clear()
    global flag
...
def text_received(message):
    ...
    bot.send_message(message.chat.id, "Кажется, у вас очепятка.\n"
                    "Может быть, вы хотели написать что-то из этого?\n\n"
                    "<i>Информация по командам:</i>\n"
                    "Чтобы сдать задание (отправить фотографии), напишите /pass\nПосле отправки фотографий, напишите /got\n"
                    "Чтобы получить альбом, напишите /send_album\n"
                    "Чтобы получить помощь, напишите /help\n\n",
                    parse_mode="HTML")
...
def buttons(call):
    if call.data.startswith('confirm'):
        ...
        bot.send_message(int(target_id),
                         ...
                         '\n\nКогда будете готовы отправить его, сначала <b>напишите /pass<\b>',
                         parse_mode='HTML')
